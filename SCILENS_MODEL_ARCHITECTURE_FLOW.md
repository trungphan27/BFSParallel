...
Hệ thống chỉ nhận **một file PDF của scientific paper**.

Không yêu cầu và không sử dụng:

- source LaTeX;
- XML;
- HTML;
- file supplementary bên ngoài PDF.

Ngôn ngữ và độ dài bản tóm tắt là cấu hình của hệ thống, không phải tài liệu
đầu vào.
...
SCIENTIFIC PAPER.pdf
        │
        ▼
[1] Kiểm tra chất lượng PDF và chọn chế độ parse
        │
        ▼
...
## 3. Công đoạn 1 — Kiểm tra PDF và chọn chế độ parse

### Vấn đề cần giải quyết

File đầu vào luôn là PDF, nhưng PDF có thể:

- có text layer chất lượng tốt;
- là bản scan hoàn toàn và phải OCR;
- có một số trang là text, một số trang là ảnh;
- bị lỗi font encoding hoặc reading order;
- chứa công thức và bảng dưới dạng ảnh;
- bị mờ, lệch trang hoặc mất ký tự.

Nếu chọn sai chế độ parse, mọi bước sau đều bị ảnh hưởng.

### Xử lý

1. Kiểm tra PDF có render được không.
2. Kiểm tra text layer, số trang và chất lượng ký tự.
3. Đo độ mờ, độ lệch, mật độ ảnh và chất lượng OCR dự kiến theo từng trang.
4. Dự đoán chất lượng text, layout, equation và table trực tiếp từ PDF.
5. Chọn một trong các đường:
   `PDF-text-layout`, `PDF-vision-OCR`, `PDF-hybrid` hoặc `manual-review`.
...
- **Khi cần huấn luyện:** một CNN/ViT nhỏ mã hóa page thumbnails, kết hợp với
  các đặc trưng PDF qua MLP để phân loại chế độ parse và dự đoán quality score.
...
- route được chọn;
- quality scores;
- page images, text layer và metadata được trích từ PDF;
...
7. Hợp nhất kết quả từ text layer và OCR theo confidence.
...
Mỗi object giữ page, bounding box, reading order và `object_id` để truy ngược
về đúng vị trí trong PDF.
...
Scientific Document Graph có typed nodes, typed edges, `object_id`, tọa độ PDF
và provenance của parser.
...
Một content plan có cấu trúc, đã chọn đủ text, equation, table cells và các
`evidence_object_id` trỏ về PDF.
...
3. Giữ `evidence_object_id` trong intermediate output.
...
Bản summary nháp có cấu trúc, evidence object IDs và các atomic claims.
...
|---|---|
| Textual claim | entailment với evidence spans trích từ PDF |
