...
*   **Mô hình (**`M`**)**: Cấu trúc DeepLabv3+ với backbone `E_img` là bộ mã hóa hình ảnh CLIP (ví dụ: ResNet-50) được đóng băng.
*   `M = (M_feat, M_cls)`. Trong đó `M_feat` là phần trích xuất đặc trưng (frozen) và `M_cls` là phần phân loại (sẽ được fine-tune).
*   **Miền nguồn (**`D_s`**)**: Tập dữ liệu huấn luyện:
    
    $$ D_s = \{(x_s, y_s) \mid x_s \in \mathbb{R}^{H\times W \times 3}, y_s \in \{0,1\}^{H\times W \times K}\} $$

*   **Miền đích (**`D_t`**)**: Tập dữ liệu chưa từng thấy, chỉ được mô tả qua một prompt (TrgPrompt). Mục tiêu là cải thiện hiệu suất trên:

    $$ D_t = \{x_t \mid x_t \in \mathbb{R}^{H\times W \times 3}\} $$
...
PØDA định nghĩa tập đặc trưng cấp thấp từ ảnh nguồn:

$$ \mathcal{F}_s = \{\mathbf{f}_s \mid \mathbf{f}_s = \text{feat-ext}(M_{feat}, \mathbf{x}_s)\} $$

Trong bài báo, lớp `Layer1` của ResNet-50 thường được sử dụng.

Embedding của prompt mục tiêu: 

$$ \text{TrgEmb} = E_{txt}(\text{TrgPrompt}) $$
...
Với `μ(·)` và `σ(·)` là giá trị trung bình (mean) và độ lệch chuẩn (standard deviation) theo kênh. `f_t` là đặc trưng phong cách mục tiêu.

Tuy nhiên, trong Zero-shot, ta không có hình ảnh mục tiêu nên không có `f_t`. Do đó, PØDA đề xuất lớp **PIN**:
...
Trong đó, `μ` và `σ` là **các biến số có thể tối ưu hóa** được dẫn dắt bởi prompt.

**Tối ưu hóa (Optimization)**:

Quá trình tối ưu hóa (khai phá phong cách) được mô tả trong **Algorithm 1** và **Figure 3**.

1.  Khởi tạo các biến phong cách: 

    $$ \boldsymbol{\mu}^0 \leftarrow \text{mean}(\mathbf{f}_s) \quad \text{và} \quad \boldsymbol{\sigma}^0 \leftarrow \text{std}(\mathbf{f}_s) $$

2.  Lặp lại `N` lần (optimization steps):
    *   Tạo đặc trưng phong cách giả định: 
    
        $$ \mathbf{f}^i_{s\rightarrow t} = \text{PIN}(\mathbf{f}_s, \boldsymbol{\mu}^{i-1}, \boldsymbol{\sigma}^{i-1}) $$
    
    *   Lấy embedding của đặc trưng giả định trong không gian CLIP: 
    
        $$ \bar{\mathbf{f}}^i_{s\rightarrow t} = \text{get-embedding}(\mathbf{f}^i_{s\rightarrow t}) $$
    
        Hàm `get-embedding` sử dụng lớp attention pooling của `E_img`.
    ...
    *   Cập nhật `μ` và `σ` thông qua quá trình hạ gradient (gradient descent) để giảm thiểu hàm mất mát trên.

Kết quả cuối cùng của quá trình lặp này là một cặp `(μ_t, σ_t)` đại diện cho phong cách mục tiêu được khai phá từ ảnh nguồn tương ứng. Tập hợp tất cả các cặp này tạo thành tập phong cách `S_{s \rightarrow t}`.

### 3.4. Tinh chỉnh để Thích ứng (Fine-tuning for Adaptation)

Quy trình tổng thể được mô tả trong **Algorithm 2**.

Sau khi có tập các phong cách mục tiêu `S_{s \rightarrow t}` (có kích thước bằng với kích thước tập dữ liệu nguồn `|D_s|`), quá trình huấn luyện bắt đầu.

Ở mỗi vòng lặp huấn luyện:
1.  Lấy một ảnh nguồn `x_s` và trích xuất đặc trưng `f_s`.
2.  Chọn ngẫu nhiên một phong cách `(μ_t, σ_t)` từ `S_{s \rightarrow t}`.
3.  Áp dụng PIN để tạo đặc trưng tăng cường: 
    
    $$ \mathbf{f}_{s\rightarrow t} = \text{PIN}(\mathbf{f}_s, \boldsymbol{\mu}_t, \boldsymbol{\sigma}_t) $$
    
4.  Truyền `f_{s \rightarrow t}` qua các lớp còn lại của `M_feat` và sau đó vào `M_cls`.
5.  Do thao tác điều chỉnh phong cách (PIN) bảo toàn nội dung ngữ nghĩa (semantic-content), nhãn nguồn `y_s` vẫn có thể được sử dụng để huấn luyện. Hàm mất mát phân đoạn thông thường (Cross Entropy Loss) được áp dụng.
6.  Chỉ trọng số của bộ phân loại `M_cls` được cập nhật trong bước truyền ngược (backward pass). `M_feat` giữ nguyên (frozen).
...
*   **Huấn luyện Source-only**: Huấn luyện `M_cls` 200k vòng lặp trên ảnh cắt ngẫu nhiên 768x768. `Layer1` bị đóng băng. Sử dụng Data Augmentation tiêu chuẩn. (Việc giữ lại `M_feat` là CLIP pre-trained weights được chứng minh là có ích, giúp chống over-fitting, theo **Table 1**).
*   **Tăng cường đặc trưng**: Đặc trưng của `Layer1` (kích thước `192x192x256`) được chọn để augment. Các biến `μ` và `σ` là vector 256 chiều (bằng số kênh của `Layer1`).
*   **Fine-tuning**: Bộ phân loại `M_cls` được fine-tune trong 2000 vòng lặp trên các đặc trưng đã được chuyển đổi.
...
*   **Số lượng phong cách được khai phá (Number of mined styles)**: Thực nghiệm chỉ ra rằng số lượng phong cách `|S_{s \rightarrow t}|` cần xấp xỉ bằng số lượng ảnh nguồn `|D_s|` để mang lại độ bao phủ phân phối (distribution coverage) tốt nhất. Quá ít phong cách sẽ dẫn đến phương sai cao.
*   **Khởi tạo khai phá phong cách (Style mining initialization)**: **Table 12** trong phụ lục chứng minh rằng việc khởi tạo `μ_0, σ_0` bằng mean và std của *chính bức ảnh nguồn* (thay vì khởi tạo bằng 0 hoặc nhiễu Gaussian ngẫu nhiên) đóng vai trò như một hình thức chính quy hóa (regularization). Điều này giúp giữ lại nội dung ngữ nghĩa (semantic preservation).
