...
\mathbf{h}_{route}
=
\operatorname{MLP}([\mathbf{h}_{page};\mathbf{x}_{meta}]).
\]
...
\mathcal{L}_{layout}
=
\lambda_{cls}\mathcal{L}_{block-cls}
+\lambda_{box}\mathcal{L}_{box}
+\lambda_{ord}\mathcal{L}_{order}
+\lambda_{head}\mathcal{L}_{heading}.
\]
...
=
\prod_{t=1}^{T}
p(l_t\mid l_{<t},\operatorname{Enc}_{vision}(I)).
\]
...
\mathbf{h}_{eq}
=
g_v\mathbf{h}_{vision}
+g_l\mathbf{h}_{latex}
+g_c\mathbf{h}_{context},
\]
...
[\mathbf{h}_{vision};\mathbf{h}_{latex};\mathbf{h}_{context}]).
\]
...
\operatorname{MLP}(\operatorname{CrossEnc}[s;eq;d]).
\]
...
\mathbf{p}_{role}
=
\sigma(\mathbf{W}_{role}\mathbf{h}_{eq}+\mathbf{b}).
\]
...
\mathcal{L}_{eq}
=
\lambda_d\mathcal{L}_{detect}
+\lambda_l\mathcal{L}_{latex}
+\lambda_g\mathcal{L}_{ground}
+\lambda_r\mathcal{L}_{role}
+\lambda_s\mathcal{L}_{salience}
+\lambda_a\mathcal{L}_{alignment}.
\]
...
\(\Delta_{impact}>0\) nghĩa là bỏ/thay component làm hiệu năng xấu đi.
...
\mathcal{L}_{table}
=
\lambda_{det}\mathcal{L}_{TATR}
+\lambda_{role}\mathcal{L}_{table-role}
+\lambda_{cell}\mathcal{L}_{cell-type}
+\lambda_{link}\mathcal{L}_{header-link}
+\lambda_{tuple}\mathcal{L}_{result-tuple}
+\lambda_{abl}\mathcal{L}_{ablation}
+\lambda_{rank}\mathcal{L}_{result-rank}.
\]
...
\mathbf{p}_{rhet}
=
\sigma(\mathbf{W}_{rhet}\mathbf{h}_{sent}+\mathbf{b}).
\]
...
=
\operatorname{MLP}_t
[\mathbf{h}_i;\mathbf{h}_j;\mathbf{e}_{width}].
\]
...
\mathcal{L}_{text}
=
\lambda_{rhet}\mathcal{L}_{rhet}
+\lambda_{ner}\mathcal{L}_{NER}
+\lambda_{rel}\mathcal{L}_{relation}
+\lambda_{claim}\mathcal{L}_{claim}
+\lambda_{contrib}\mathcal{L}_{contribution}
+\lambda_{limit}\mathcal{L}_{limitation}.
\]
...
\mathbf{q}_t
=
\mathbf{W}^{Q}_{\tau(t)}\mathbf{h}_t,
\]

\[
\mathbf{k}_s^{r}
=
\mathbf{W}^{K}_{\tau(s),r}\mathbf{h}_s,
\qquad
\mathbf{v}_s^{r}
=
\mathbf{W}^{V}_{\tau(s),r}\mathbf{h}_s.
\]
...
\left(
\frac{\mathbf{q}_t^\top
\mathbf{W}^{ATT}_{r}\mathbf{k}_s^r}
{\sqrt{d}}
+b_r
\right).
\]
...
=
\alpha_{s\rightarrow t}^{r}
\mathbf{W}^{MSG}_{r}\mathbf{v}_s^r.
\]
...
\[
\mathcal{L}_{CE-align}
=
-\log
\frac{\exp(sim(c,e^+)/\tau)}
{\exp(sim(c,e^+)/\tau)+
\sum_{e^-}\exp(sim(c,e^-)/\tau)}.
\]
...
p_v^{sal}
=
\sigma(\operatorname{MLP}_{sal}(\mathbf{h}_v)).
\]
...
\mathcal{L}_{verify}
=
\lambda_{nli}\mathcal{L}_{NLI}
+\lambda_{type}\mathcal{L}_{claim-type}
+\lambda_{attr}\mathcal{L}_{attribution}
+\lambda_{conf}\mathcal{L}_{confidence}.
\]
...
action(c)=
\begin{cases}
accept, & C(c)\ge \tau_{high}\\
repair, & \tau_{low}\le C(c)<\tau_{high}\\
abstain, & C(c)<\tau_{low}.
\end{cases}
\]
...
=
\mathcal{L}_{MLM}
+\mathcal{L}_{eq-align}
+\mathcal{L}_{cell-header}
+\mathcal{L}_{cross-modal}.
\]
...
\[
\mathcal{L}_{graph-pre}
=
\lambda_1\mathcal{L}_{MNM}
+\lambda_2\mathcal{L}_{edge}
+\lambda_3\mathcal{L}_{claim-evidence}
+\lambda_4\mathcal{L}_{eq-context}
+\lambda_5\mathcal{L}_{cell-header}.
\]
